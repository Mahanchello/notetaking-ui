...
// let variables;
  const [updateNote, {variables}] = useMutation(UPDATE_NOTE);

  if (loading) return <div>Fetching note</div>;
    if(error) return <div>Error fetching note</div>;

  // set the result gotten from the GraphQL server  into the note variable.
  const note = data;
  ...
  return (
    ...
    //set the title of the note to the title in the state, if note's available set to the original title goten from the GraphQL server
        //set the content of the note to the content in the state, if note's avaialble set to the original content gotten from the GraphQL server
        //pass the id, title and content as variables to the  UPDATE_NOTE mutation.

        let id = note.getNote._id;
        console.log(id)
        updateNote({
          variables: {
    ...
    notify.show("Note was edited successfully", "success")
          ...
          <div className="field">
            ...
            <div className="control">
                <input className="input" type="text" name="title" placeholder="Note title"
